...
class ScienceParse:
  ...
  def get_content_words(self):
    """Returns cached split of paper content to avoid redundant splitting."""
    if self._cached_content_words is not None:
      return self._cached_content_words
    self._cached_content_words = self.get_paper_content().split(" ")
    return self._cached_content_words

  # get_tagged_paper_content removed — spacy dependency was heavy and unused

  def get_frequent_words_proportion(self, hfws, most_frequent_words, least_frequent_words):
    content = self.get_content_words()
  ...
  def get_num_uniq_words(self):
    return len(set(self.get_content_words()))
  ...
  def get_contains_appendix(self):
    content_words = self.get_content_words()
    return int("Appendix" in content_words)

  # publishing a dataset / code
  # Duplicate get_contains_appendix removed
